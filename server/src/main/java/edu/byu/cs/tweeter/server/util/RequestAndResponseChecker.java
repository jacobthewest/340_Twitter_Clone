...
public class RequestAndResponseChecker {
    ...
    public void checkSameUser(User user, User followUser) {
        if(user.equals(followUser)) {
            throw new RuntimeException("[BadRequest] User and FollowUser cannot be the same.");
        }
    }
    ...
    public void checkStatus(Status status) {
        ...
        if(status.getUser().getAlias() == null || status.getUser().getAlias().equals("")) {
